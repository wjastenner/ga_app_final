    ...
    req.on('data', async function (data) {
        ...
        for (var filter in filters) {
            if (filters[filter]) {
                ...
                sqlStatement += " " + filter + " = '" + filters[filter] + "'";
                filterCount += 1;
            }
        }
...
app.post('/get_fault', async function (req, res) {
	...
	req.on('data', async function (data) {
		var json = JSON.parse(data);
		console.log(json);
		sqlStatement = "SELECT * FROM fault WHERE faultno = " + json;
		console.log(sqlStatement);
		try {
			const sqlQueryResult = await client.query(sqlStatement);
			result = sqlQueryResult.rows;
...
app.post('/get_assigned_faults', async function (req, res) {
    ...
    req.on('data', async function (data) {
        var result;
        var json = JSON.parse(data);
        sqlStatement = 'SELECT * FROM fault WHERE assignedTo =' + json.userID + ' ORDER BY dateReported ASC;';
        console.log(sqlStatement);
        try {
            const SQLQueryResult = await client.query(sqlStatement);
            result = SQLQueryResult.rows;
...
app.post('/get_carriage_faults', async function (req, res) {
    ...
    req.on('data', async function (data) {
        var result;
        var json = JSON.parse(data);
        sqlStatement = "SELECT faultNo, carriageNo, category, location, faultDesc, dateReported FROM fault WHERE carriageno =" + json + " AND (status = 'I' OR status = 'R') ORDER BY dateReported ASC;";
        console.log(sqlStatement);
        try {
            const SQLQueryResult = await client.query(sqlStatement);
            result = SQLQueryResult.rows;
